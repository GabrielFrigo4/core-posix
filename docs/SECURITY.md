...
Diretrizes de segurança para a suíte de ferramentas do repositório `core-posix` (`rtdo` e `rtgo`).

---

## 🎯 Modelo de Ameaças

Utilitários que operam com bit SUID root ativo constituem pontos de máxima criticidade em sistemas Unix. O modelo de segurança adota uma postura de defesa em camadas (_defense-in-depth_).
...
- **Ameaça:** Despejos de memória (_core dumps_) ou inspeção residual da pilha expondo senhas em texto puro.
