...
{
	return memcmp(header->riff_id, "RIFF", 4) == 0 && memcmp(header->wave_id, "WAVE", 4) == 0 &&
	       memcmp(header->fmt_id, "fmt ", 4) == 0 && header->audio_format == 1 &&
...
int main(int argc, char **argv)
{
	...
	int dsp_fd = oss_open_device(
	    device, O_WRONLY, oss_format, header.num_channels, header.sample_rate
	);
	if (dsp_fd < 0)
	{
	...
	printf(
	    "Tocando '%s' (%u Hz, %u bits, %u canais)...\n",
	    filename,
	    header.sample_rate,
	    header.bits_per_sample,
	    header.num_channels
